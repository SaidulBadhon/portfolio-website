...
/** Splits a separated list (e.g. comma-separated tags), dropping empty entries. */
const splitList = (text: string, separator: string) =>
  text
    .split(separator)
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent fullScreen>
        {/* The dialog unmounts its content when closed, so each open starts a fresh form. */}
        <ProjectForm mode={mode} onClose={onClose} onSuccess={onSuccess} />
      </DialogContent>
    </Dialog>
  );
}
...
function ProjectForm({
  ...
  // Captured on open: the parent resets `mode` while the dialog animates out.
  const [initial] = useState(() =>
    mode === "new" ? emptyProject() : { ...mode }
  );
  const [editingId] = useState(mode === "new" ? null : mode.id);
  const [form, setForm] = useState<Partial<ProjectItem>>(initial);
  // Lists are edited as text and parsed on use, so separators can be typed.
  ...
  const features = splitList(featuresText, "\n");
  ...
  const updateTechnologies = (value: string[]) => {
  ...
  const save = async () => {
    ...
    if (editingId === null && !SLUG_PATTERN.test(id)) {
      setError("ID may only contain letters, numbers, and single hyphens (e.g. my-project).");
      return;
    }
    setSaving(true);
    setError(null);
    try {
      const payload: Partial<ProjectItem> = { ...form, id, tags, features };
      ...
      if (editingId === null) {
        ...
        await adminApi.projects.update(editingId, payload);
      }
      onClose();
      onSuccess?.();
    } catch (e) {
  ...
  const addTechnology = () => {
    ...
    updateTechnologies([...technologies, value]);
    setTechInput("");
  };

  const removeTechnology = (index: number) => {
    updateTechnologies(technologies.filter((_, i) => i !== index));
  };
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      if (generated && typeof generated === "object") {
        ...
        if (generated.tags?.length > 0) setTagsText(generated.tags.join(", "));
        if (generated.features?.length > 0) setFeaturesText(generated.features.join("\n"));
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <DialogFooter
        ...
        className="-mx-6 -mb-6 shrink-0 rounded-none"
      >
